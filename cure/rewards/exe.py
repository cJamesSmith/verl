...
def run_scripts_with_chunk(
    code_list, test_input_list, time_limit_list, worker, num_chunks
):
    ...
    for start, end in chunks:
        ...
        sub_exe_results = run_scripts_with_timeout(
            sub_code_list, sub_test_input_list, sub_time_limit_list, worker
        )
...
def execute_scripts(num_chunks):
    ...
    exe_results = run_scripts_with_chunk(
        code_list, case_input_list, time_limit_list, worker, num_chunks
    )
    ...
    for i in range(len(index_list)):
        ...
        data[index_i]["all_case_bool_table"][j][k] = test_if_eq(
            exe_results[i], case_output_list[i]
        )

    # get stats
    stats_single = {"BoN_score": 0, "BoN_num": 0, "BoN_acc_score": 0, "BoN_acc_num": 0}
    stats = []
    ...
    for i in range(len(data)):
        ...
        if len(correct_code_list) > 0:
            ...
            wrong_code_list = [
                j
                for j in range(all_case_table_i.shape[0])
                if j not in correct_code_list
            ]
            wrong_case_list = [
                j
                for j in range(all_case_table_i.shape[1])
                if j not in correct_case_list
            ]
            if len(wrong_code_list) > 0:
                if len(correct_case_list) > 0:
                    wrong_code_correct_case_table_i = all_case_table_i[
                        wrong_code_list, :
                    ][:, correct_case_list].copy()
                ...
                if len(wrong_case_list) > 0:
                    wrong_code_wrong_case_table_i = all_case_table_i[
                        wrong_code_list, :
                    ][:, wrong_case_list].copy()
        ...
        for scale_num_code, scale_num_case in scale_tuple_list:
            ...
            stats[index_id]["BoN_score"] = stats[index_id]["BoN_score"] + int(
                all(sub_test_table_i)
            )
            stats[index_id]["BoN_num"] = stats[index_id]["BoN_num"] + 1
            stats[index_id]["BoN_acc_score"] = (
                stats[index_id]["BoN_acc_score"] + np.sum(sub_test_table_i).item()
            )
            stats[index_id]["BoN_acc_num"] = stats[index_id]["BoN_acc_num"] + len(
                sub_test_table_i
            )
            assert int(all(sub_test_table_i)) / 1 <= np.sum(
                sub_test_table_i
