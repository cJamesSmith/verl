...
class TensorHelper:
    ...
    def cut_to_effective_length(self, tensor_dict: Dict[str, torch.Tensor], keys: List[str], cut_left: bool = True) -> Dict[str, torch.Tensor]:
        """
        Cut the tensors in tensor_dict to their effective lengths based on the attention masks.
        """
    ...
    def convert_pad_structure(self, tensor: torch.Tensor, pad_to_left: bool = True) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Convert the padding structure of the input tensor, and return the sorted tensor with indices.
        """
    ...
    def create_attention_mask(self, input_ids: torch.Tensor) -> torch.Tensor:
        """
        Create attention mask for the input_ids tensor.
        """
        return torch.where(input_ids != self.config.pad_token_id, 1, 0)

    def create_position_ids(self, attention_mask: torch.Tensor) -> torch.Tensor:
        """
        Create position ids for the input_ids tensor based on attention mask.
        """
    ...
    def _example_level_pad(self, responses: torch.Tensor, responses_str: List[str], active_mask: torch.Tensor) -> Tuple[torch.Tensor, List[str]]:
        """
        Pad the responses for non-active examples with pad tokens.
        """
        assert active_mask.sum() == responses.shape[0], f"active_mask.sum() ({active_mask.sum()}) != responses.shape[0] ({responses.shape[0]})"
        # Create masked responses tensor
        batch_size = active_mask.shape[0]
        seq_len = responses.shape[1]
        padded_responses = torch.full((batch_size, seq_len), self.config.pad_token_id, dtype=responses.dtype, device=responses.device)
        padded_responses[active_mask] = responses

        # create masked responses_str list
        padded_responses_str = [""] * batch_size

        for i, is_active in enumerate(active_mask):
            if is_active:
                padded_responses_str[i] = responses_str.pop(0)
...
class AgentHelper:
    def __init__(self, tokenizer: AutoTokenizer, actor_rollout_wg, config: GenerationConfig):
        ...
        self.prompt_dict = {"no_tool_prompt": "\n", "final_prompt": "\n"}
        self.error_n_line = 1
    ...
    def _postprocess_responses(self, responses: torch.Tensor) -> Tuple[torch.Tensor, List[str]]:
        ...
        # Drop the responses after the python code block
    ...
    def _process_next_obs(self, next_obs: List[str]) -> torch.Tensor:
        """
        Process the next observations from environment.
        """
        next_obs_ids = self.tokenizer(next_obs, add_special_tokens=False, return_tensors="pt", padding="longest")["input_ids"]

        if next_obs_ids.shape[1] > self.config.max_obs_length:
            print(f"[WARN] observation {next_obs_ids.shape[1]} exceeds max_obs_length {self.config.max_obs_length}, truncating.")
    ...
    def _update_rolling_state(self, rollings: DataProto, cur_responses: torch.Tensor, next_obs_ids: torch.Tensor) -> DataProto:
        """
        Update the rolling state with new responses and new observations.
        """
        # Concatenate and handling padding
        ...
        new_rollings: DataProto = DataProto.from_dict(
            {
    ...
    def _info_masked_concatenate_with_padding(self, prompt: torch.Tensor, prompt_with_mask: torch.Tensor, response: torch.Tensor, info: torch.Tensor, pad_to_left: bool = True) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Concatenate tensors and handle padding. Additionally, create a mask (info_mask) to cover the information block if exists.
        `info` may be the next obs
        """
        ...
        padded_tensor_with_mask = concatenated_with_mask.gather(1, sorted_indices)

        return padded_tensor, padded_tensor_with_mask

    def _update_right_side(self, right_side: Dict, cur_responses: torch.Tensor, next_obs_ids: torch.Tensor) -> Dict:
        """
        Update the right side state
        """
        if next_obs_ids is not None:
            responses, responses_with_info_mask = self._info_masked_concatenate_with_padding(right_side["responses"], right_side["responses_with_info_mask"], cur_responses, next_obs_ids, pad_to_left=False)
        else:
            responses, responses_with_info_mask = self._info_masked_concatenate_with_padding(right_side["responses"], right_side["responses_with_info_mask"], cur_responses, None, pad_to_left=False)
    ...
    def _generate_with_gpu_padding(self, active_batch: DataProto) -> DataProto:
        ...
        for key, value in active_batch.batch.items():
            pad_tensor = value[0:1].repeat(pad_size, *[1 for _ in range(len(value.shape) - 1)])
            padded_batch[key] = torch.cat([value, pad_tensor], dim=0)
        ...
        # generate sequences with padded batch
        padded_output = self.actor_rollout_wg.generate_sequences(padded_active_batch)

        # remove padding sequences from output
        trimmed_batch = {k: v[:-pad_size] for k, v in padded_output.batch.items()}
    ...
    def run_llm_loop(self, gen_batch: DataProto, initial_input_ids: torch.Tensor, timeout: int = 5):
        """
        Run main LLM gen loop
        """
        ...
        void_turn_mask = torch.ones(batch_size * self.config.rollout_n, dtype=torch.bool)  # if void turn, set to false
        turns_stats = torch.ones(batch_size * self.config.rollout_n, dtype=torch.int)
        use_code_stats = torch.zeros(batch_size * self.config.rollout_n, dtype=torch.int)
        valid_code_stats = torch.zeros(batch_size * self.config.rollout_n, dtype=torch.int)
        success_code_lines = []
        fail_code_lines = []
        ...
        for step in range(self.config.max_turns):
            if not active_mask.sum():
                print(f"All rollouts finished at step {step}.")
                break
            rollings.batch = self.tensor_fn.cut_to_effective_length(rollings.batch, keys=["input_ids", "attention_mask", "position_ids"])
            ...
            if step != 0:
                ...
                if self.config.rollout_n == 1:
                    ...
                    for k, v in rollings.batch.items():
                        repeated_rollings_dict[k] = v.repeat_interleave(self.config.rollout_n, dim=0)
                    rollings = DataProto.from_dict(repeated_rollings_dict)
                    for k, v in original_left_side.items():
                        original_left_side[k] = v.repeat_interleave(self.config.rollout_n, dim=0)
                    for k, v in original_right_side.items():
                        original_right_side[k] = v.repeat_interleave(self.config.rollout_n, dim=0)
            ...
            # Execute code and get next observations
            next_obs, dones, is_void_turn, code_info = self.execute_predictions(responses_str, active_mask)
            curr_active_mask = torch.tensor([not done for done in dones], dtype=torch.bool)
            active_mask = active_mask * curr_active_mask
            void_turn_mask = void_turn_mask * torch.tensor([not v for v in is_void_turn], dtype=torch.bool)
        ...
        return self._compose_final_output(original_left_side, original_right_side, void_turn_mask, meta_info)
...
"""
        ...
        if tasks:
            sandbox_success, sandbox_stdout, sandbox_stderr = asyncio.run(parallel_sandbox(tasks, num_processes=256))
        ...
        print(f"[debug] void turn number: {sum(is_void_turn)} out of {active_mask.sum()} samples")
